...
export function IncidentCard({ incident, onClick, onAssign, selected, index = 0 }: IncidentCardProps) {
  ...
  return (
    ...
    >
      {incident.priority === "critical" && (
        <span className="absolute top-0 left-0 right-0 h-0.5 bg-critical animate-pulse z-10" />
      )}
      ...
      <div className="px-3 pb-3 flex items-center justify-between">
        <span className="text-[10px] uppercase tracking-wider text-muted-foreground">{incident.needType} · {incident.status.replace("_", " ")}</span>
        {onAssign ? (
          <button
            onClick={(e) => { e.stopPropagation(); onAssign(); }}
            className="text-xs font-medium text-primary flex items-center gap-1 group-hover:gap-2 transition-all hover:underline"
          >
